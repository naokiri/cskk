...
impl CommandHandler for KanaCompositionHandler {
    ...
    fn get_instruction(&self, key_event: &KeyEvent, current_state: &CskkState, is_delegated: bool) -> Vec<Instruction> {
        ...
        // } else
        if !is_delegated && (xkb::keysyms::KEY_Return == symbol || (xkb::keysyms::KEY_j == symbol && modifier.contains(SkkKeyModifier::CONTROL))) {
            ...
            return instructions;
        } else if !is_delegated && (xkb::keysyms::KEY_g == symbol && modifier.contains(SkkKeyModifier::CONTROL)) {
            // Abort
            instructions.push(Instruction::Abort);
            instructions.push(ChangeCompositionMode { composition_mode: CompositionMode::PreComposition, delegate: false });
            instructions.push(Instruction::FinishConsumingKeyEvent);
            return instructions;
        ...
        } else if !is_delegated && (xkb::keysyms::KEY_a <= symbol && symbol <= xkb::keysyms::KEY_z) || (xkb::keysyms::KEY_A <= symbol && symbol <= xkb::keysyms::KEY_Z) {
            // 現在の変換で確定させ、次のモードでキー入力を処理させる。 "I s i space k" の kのような時。
            // 後続で入力として処理させるので、Finishしない。
            instructions.push(Instruction::ConfirmComposition);
            instructions.push(ChangeCompositionMode { composition_mode: CompositionMode::Direct, delegate: true });
        }
